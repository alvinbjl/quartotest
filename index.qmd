...
```{r echo=FALSE}
mapview(dis_sf, alpha.region = 0, layer.name = "Districts") + 
    mapview(mkm_sf, 
          alpha.region = 0, 
          color = "grey", 
          layer.name = "Mukims") +
...
mapview(sch_sf, zcol = "Sector", layer.name = "Sector") + 
  mapview(sch_sf, zcol = "Cluster", layer.name = "Cluster")
```
...
Table: Count of schools, classes, and students by cluster
...
Table: Summary of Student-Teacher Ratio by District
...
Table: Summary of Student-Teacher Ratio by Sector
...
![](images/student_teacher_ratio.pdf){width="100%"}
...
![](images/autocorrelation.pdf){width="100%"}
...
This finding supports the presence of a moderate to strong clustering tendency, implying that mukims with a similar number of schools, whether high or low, are geographically close to each other.The results of the Global Moran’s I test align with our visual observation as observed in figure below.

```{r echo=FALSE}
mapview(mkm_sch, zcol = "count_of_schools", layer.name = "School Count")
```

If not clustered, it would look something like:
```{r echo=FALSE}
mkm_sch %>% 
  mutate(rand_count = sample(count_of_schools)) %>% 
  mapview(zcol = "rand_count") 
```

Visually, we also see that mukims with more schools (brighter colors) are concentrated near the coastal regions. We will verify this using local spatial autocorrelation in the following subsection.

## Locations of School Clusters

![](images/sch_lisa.pdf){}

As highlighted in Figure, the primary concentration of schools is located in central **Mukim Brunei-Muara**, the capital district of Brunei. This is unsurprising given its status as the nation’s capital, urban, and administrative center. Other notable clusters outside the capital are located in:
...
Due to the moderate $R^2$ value, we now attempt to conduct a visual inspection using Figure and Figure . Although it seems to suggest a general alignment between schools and population hotspots by kampong level, a more detailed comparison reveals some notable patterns. Namely, when we examine the top 10 kampongs by school count and by population in Table only three kampongs, namely **Kg. Mata-Mata**, **Kg. Panaga**, and **Kg. Sungai Akar**, are shared across both lists.
...
mapview(kpg_sch, zcol = "count_of_schools", layer.name = "Kampongs")
```

<br>
Table: Top 10 Kampongs by School Count and Population

| | **Kampong**                | **Schools Count** | **Kampong**                | **Population** |
|---|----------------------------|-------------------|----------------------------|----------------|
...
<br>
Table: 10 Mukims by School Count and Population

| | **Mukim**                  | **Schools Count** | **Mukim**                  | **Population** |
|---|----------------------------|-------------------|----------------------------|----------------|
